			...
			if ( $product && taxonomy_exists( $attribute ) ) {
                ...
                foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options ) ) {
					    $pos  = strpos($term->taxonomy, 'pa_fastening');
                        $pos2 = strpos($term->taxonomy, 'pa_material-type');
                        if ($term->price == '') {$term->price = 0;}
                        if ($pos === false) {
                            if ($pos2 === false) {
                                $html .= '<option value="' . esc_attr( $term->slug ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) . '</option>';
                            } else {
                                $html .= '<option value="' . esc_attr( $term->slug ) . '" data-price="'.$term->price.'" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) . '</option>';
                            }
                        } else {
                          $html .= '<option value="' . esc_attr( $term->slug ) . '" data-size="'.$term->description.'" ' .'" data-price="'.$term->price.'" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) . '</option>';
                        }
					}
				}
			} else {
