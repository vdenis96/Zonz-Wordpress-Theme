	...
	<div class="product_wrapper">
				...
				<a href="<?php the_permalink(); ?>">
					...
					<?php
						if ( has_post_thumbnail( $post->ID ) ) { 	
							...
							//echo  get_the_post_thumbnail( $post->ID, array(263,175));
							echo  '<img src="'.get_the_post_thumbnail_url( $post->ID, array(263,175)).'" alt="">';
						}else{
        ...
        <?php if ( (isset($mr_tailor_theme_options['catalog_mode'])) && ($mr_tailor_theme_options['catalog_mode'] == 0) ) : ?>
			<?php if ( !$product->is_in_stock() && !empty($mr_tailor_theme_options['out_of_stock_text'])) : ?>            
                <div class="out_of_stock_badge_loop <?php if (!$product->is_on_sale()) : ?>first_position<?php endif; ?>"><?php echo __($mr_tailor_theme_options['out_of_stock_text'], 'getbowtied'); ?></div>            
            <?php endif; ?>
        <?php endif; ?>
