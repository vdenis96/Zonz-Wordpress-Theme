...
<div class="images" style="width: 100% !important">
    ...
    <div class="product_images">
        ...
        <div id="product-images-carousel" class="owl-carousel">
            ...
            ?>
            <!--Variation Images Start-->
            <?php
            if($product->is_type('variable')){
                $product_variations = $product->get_available_variations();
                foreach($product_variations as $product_variation) {
                    if($product_variation['image_id'] != get_post_thumbnail_id()){
                    $product_attribute_name = implode('-', $product_variation['attributes']);
                    ?>
                    <div class="owl-product-variation-image product-variation-<?php echo $product_attribute_name ?> <?php echo $zoom_class; ?>">
                        <a data-fresco-group="product-gallery" data-fresco-options="fit: 'width'" class="<?php echo $modal_class; ?>" href="<?php echo esc_url($product_variation['image']['src']); ?>">
                            <img src="<?php echo esc_url($product_variation['image']['src']); ?>" data-src="<?php echo esc_url($product_variation['image']['src']); ?>" class="lazyOwl" alt="">
                            <?php echo $plus_button; ?>
                        </a>
...
<div class="wrapper-product_thumbnails">
...
?>
<script>
function bevestigingsmateriaal() {
    ...
    if (z==0) {
        ...
        jQuery('.canvas_block').hide();
        jQuery('.no_canvas_block').hide();
        ...
        if (jQuery('.before_bev').length) {
            ...
            jQuery('#pa_bevestigingsmateriaal-test').parents('.variations_lines').before('<div class="before_bev">'+
                    '<img src="<?php echo $image_link;?>"><label>'+
...
//            }
//        });
        jQuery(document).on('click','#pa_bepaal-je-afmeting', function(){
            bevestigingsmateriaal();
            jQuery('.sizes_enrty input').val('');
            jQuery(".canvas_block input[type!='button']").val('').change();
            canv_sides_check();
            sides_check();
        });
