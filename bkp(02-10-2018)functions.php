...
function custom_woocommerce_order_add_product($id, $item_id, $product, $qty, $args){
    if($attr =  $_SESSION['productAttr'][$product->id.'_'.$qty]){
        $attr['area'] = str_replace(',','<br />',$attr['area']);
        wc_add_order_item_meta( $item_id, 'Oppervlakte', $attr['area']);
    }
}
add_action( 'woocommerce_order_add_product', 'custom_woocommerce_order_add_product', 10, 5 );
