...
function filter_gateways($gateways){
    ...
    foreach ($woocommerce->cart->cart_contents as $key => $values ) {
        ...
        $product_category_ids = wc_get_product_term_ids($values['product_id'], 'product_cat');
        //echo '<pre>';print_r($product_category_ids);
        ...
        foreach ($custom_product_cat_ids as $key => $custom_product_cat_id) {
            if(in_array($custom_product_cat_id, $product_category_ids)){
                $custom_product = true;
                break;
            }  
        }
...
add_action( 'woocommerce_before_shop_loop', 'custom_woocommerce_redirect_if_single_product_in_category', 9 );
...
function custom_order_status_icon() {
    ...
    ?> <style>
        ...
        .column-order_status mark.beoordeelbestelli::after  {
            content: url(<?php echo get_site_url(); ?>/wp-content/uploads/2016/03/status-1-1.png);
        }
        .column-order_status mark.beoordeelbestelli::after  {
...
function custom_shortcode_atts_wpcf7_filter( $out, $pairs, $atts ) {
    if ( isset( $atts['product_name'] ) ) {
        $out['product_name'] = $atts['product_name'];
    }
    return $out;
}
...
$add_imageprice_taxonomies = array(
    ...
    "pa_doeksoort-recht2",
    "pa_kies-doeksoort",
    "pa_doeksoort-gecurvd",
    "pa_stap-4-bevestiging-hoek-a",
    "pa_stap-5-bevestiging-hoek-b",
    "pa_stap-6-bevestiging-hoek-c",
    "pa_stap-5-bevestiging-hoek-a",
    "pa_stap-6-bevestiging-hoek-b",
    "pa_stap-7-bevestiging-hoek-c"
    );
...
$add_price_taxonomies = array(
    ...
    "pa_stap-6-bevestiging-hoek-b",
    "pa_stap-7-bevestiging-hoek-c",
    "pa_stap-7-bevestiging-hoek-d",
    "pa_stap-8-bevestiging-hoek-d"
    );
...
// oleg_b2b
/* To show full and discounted prices on Cart page
 * 
Prices for some items are not OK.
But Total line looks good:
...
Totaal 	
€ 507,63
(inclusief € 88,10 21% BTW)

 */
 if (current_user_can('b2b') ) {
		//$finalPrice = '<del>' . round((100*$finalPrice/85),2) . '</del> ' . $finalPrice . ' 222';
		$finalPrice = round(($finalPrice*85/100),2);
...
// oleg_b2b
    ...
    return number_format($finalPrice,2,',','') . ' function custom_woocommerce_cart_item_price';
}
            ...
            if ( $product && taxonomy_exists( $attribute ) ) {
                ...
                foreach ( $terms as $term ) {
                    ...
                    if ( in_array( $term->slug, $options, true ) ) {
                        ...
                        $pos2_1 = strpos($term->taxonomy, 'pa_stap-4-bevestiging-hoek-a'); //add image and price doeksoort
                        $pos2_2 = strpos($term->taxonomy, 'pa_stap-5-bevestiging-hoek-b');
                        $pos2_3 = strpos($term->taxonomy, 'pa_stap-6-bevestiging-hoek-c');
                        $pos2_4 = strpos($term->taxonomy, 'pa_stap-5-bevestiging-hoek-a');
                        $pos2_5 = strpos($term->taxonomy, 'pa_stap-6-bevestiging-hoek-b');
                        $pos2_6 = strpos($term->taxonomy, 'pa_stap-7-bevestiging-hoek-c');
                        $pos2_7 = strpos($term->taxonomy, 'pa_stap-7-bevestiging-hoek-d');
                        $pos2_8 = strpos($term->taxonomy, 'pa_stap-8-bevestiging-hoek-d');
                        ...
                        if ($pos === false) {
                            if ($pos2_8 === false && $pos2_7 === false && $pos2_6 === false && $pos2_5 === false && $pos2_4 === false && $pos2_3 === false && $pos2_2 ===false && $pos2_1 === false && $pos2 === false) {
                                if ($pos3 === false) {
...
add_filter('get_post_metadata', 'remove_query_from_vc_css', 10, 4);
function remove_query_from_vc_css($value, $object_id, $meta_key, $single) {
    if ($meta_key!='_wpb_shortcodes_custom_css' || is_admin()) {
        return $value;
    }
    remove_filter('get_post_metadata', 'remove_query_from_vc_css', 10);
...
function product_category_add_new_meta_field() {
    ?>
    <?php wp_nonce_field( basename( __FILE__ ), 'product_cat_details_nonce' ); ?>
    <div class="form-field">
        <label for="term_meta[bottom_title]"><?php _e('Bottom Title', 'woocommerce'); ?></label>
        <input type="text" name="term_meta[bottom_title]" id="term_meta[bottom_title]">
        <p class="description"><?php _e('Enter a bottom title', 'woocommerce'); ?></p>
    </div>
    <div class="form-field">
        <label for="pro_cat_bottom_desc"><?php _e('Bottom Description', 'woocommerce'); ?></label>
        <textarea name="pro_cat_bottom_desc" id="pro_cat_bottom_desc" rows="5"></textarea>
        <p class="description"><?php _e('Enter a bottom description', 'woocommerce'); ?></p>
    </div>
    <?php
}

add_action('product_cat_add_form_fields', 'product_category_add_new_meta_field', 10, 2);

//Product Cat Edit page
function product_category_edit_meta_field($term) {
    ...
    ?>
    <tr class="form-field">
        <th scope="row" valign="top"><label for="term_meta[bottom_title]"><?php _e('Bottom Title', 'woocommerce'); ?></label></th>
        <td>
            <input type="text" name="term_meta[bottom_title]" id="term_meta[bottom_title]" value="<?php echo esc_attr($term_meta['bottom_title']) ? esc_attr($term_meta['bottom_title']) : ''; ?>">
            <p class="description"><?php _e('Enter a bottom title', 'woocommerce'); ?></p>
        </td>
    </tr>
    <tr class="form-field">
        <th scope="row" valign="top"><label for="term_meta[bottom_desc]"><?php _e('Bottom Description', 'woocommerce'); ?></label></th>
        <td>
            <?php wp_nonce_field( basename( __FILE__ ), 'product_cat_details_nonce' ); ?>
            <?php
                $pro_cat_bottom_desc = get_term_meta( $term_id, 'pro_cat_bottom_desc', true );
                if ( ! $pro_cat_bottom_desc ) {
                    $pro_cat_bottom_desc = '';
                }
                $settings = array( 'textarea_name' => 'pro_cat_bottom_desc' );
                $editor_id = 'pro_cat_bottom_desc';
                wp_editor( $pro_cat_bottom_desc, $editor_id, $settings );
            ?>
            <p class="description"><?php _e('Enter a bottom description', 'woocommerce'); ?></p>
        </td>
    </tr>
...
add_action('product_cat_edit_form_fields', 'product_category_edit_meta_field');
...
function save_product_category_custom_meta($term_id) {
    ...
    if ( ! isset( $_POST['product_cat_details_nonce'] ) || ! wp_verify_nonce( $_POST['product_cat_details_nonce'], basename( __FILE__ ) ) ) {
        return;
    }
    $old_details = get_term_meta( $term_id, 'pro_cat_bottom_desc', true );
    $new_details = isset( $_POST['pro_cat_bottom_desc'] ) ? $_POST['pro_cat_bottom_desc'] : '';
    if ( $old_details && '' === $new_details ) {
        delete_term_meta( $term_id, 'pro_cat_bottom_desc' );
...
add_action('edited_product_cat', 'save_product_category_custom_meta', 10, 2);
add_action('create_product_cat', 'save_product_category_custom_meta', 10, 2);



add_filter('woocommerce_email_recipient_customer_completed_order', 'custom_conditional_email_notification', 10, 2);
function custom_conditional_email_notification( $recipient, $order){
	//if( is_admin() ) return $recipient;

    $disable = 1;
    $product_categories = array( 401 );
    ...
    foreach ( $order->get_items() as $item ) {
        ...
        if( has_term( $product_categories, 'product_cat', $product_id ) && $disable == 1 ){
            $disable = 1;
        }else{
        	$disable = 0;
        }
...
add_filter( 'woocommerce_shipping_free_shipping_is_available', 'custom_disable_free_shipping', 20, 1 );
function custom_disable_free_shipping( $is_available ) {
	global $woocommerce;
	// set the product ids that are ineligible
	$ineligible = array( '1518194' );
	// get cart contents
...
add_filter( 'woocommerce_package_rates', 'custom_disable_flat_shipping', 100 );
function custom_disable_flat_shipping( $rates ) {
	$free = array();
...
// disable setting of woocommerce_recently_viewed cookie for easier caching with varnish
remove_action( 'template_redirect', 'wc_track_product_view', 20 );


//add_action('wp_footer', 'test_mail');
function test_mail(){
    $to = 'tukhikyanpanos@gmail.com';
    $subject = 'Test Mail';
    $body = 'The email body content';
    $headers = array('Content-Type: text/html; charset=UTF-8');
     
    if(wp_mail( $to, $subject, $body, $headers )){
        echo '<div style="display:none;">Sent</div>';
    }else{
        echo '<div style="display:none;">Sorry</div>';
    }
}

//show_admin_bar (true);

/* oleg 010720 Allow customers to access wp-admin - but anyway code not working for Product pages
if ( current_user_can( 'manage_options' ) ) {
 add_filter('show_admin_bar', '__return_true');
}

add_action('after_setup_theme', 'add_admin_bar', 100);
add_action('init', 'add_admin_bar', 100);
...
*/ 
/**
 * oleg 010720 Allow customers to access wp-admin
 */
...
/* oleg 010720 Applying conditionally a discount for a specific user role 
* instead of Prices by User Role for WooCommerce plugin which has performance issues

Works ok.  
Discount is shown on Checkout page like: 
Subtotaal 	€ 597,21
Verzending 	Gratis verzending	
Discount (15%) 	€ 89,58
Totaal 	€ 507,63 (inclusief € 88,10 21% BTW)
...
function discount_based_on_user_role( $cart ) {
    if ( is_admin() && ! defined( 'DOING_AJAX' ) )
        return; // Exit
 //error_log('000 current_user_can b2b');
 //error_log(print_r(current_user_can('b2b'),true));
...
add_filter( 'woocommerce_get_price_html', 'change_displayed_sale_price_html', 100, 2 );
function change_displayed_sale_price_html( $price, $product ) {
 ...
 $percentage = 15;
 $full_price = (float) $product->get_price(); 
 $discounted_price = round($full_price * (100 - $percentage)/100,2); 
 //$price = sprintf( __('<del>€ %s</del> $price= %s $discounted_price= € %s inclusief BTW', 'woocommerce'), $full_price, $price, $discounted_price );
 $price = sprintf( __('<del>€ %s</del> € %s inclusief BTW', 'woocommerce'), $full_price, $discounted_price );
...
/* oleg18082020 For debugging - List functions attached to an action
http://www.damiencarbery.com/2017/06/list-functions-attached-to-an-action/
*/

add_action( 'wp_footer', 'wp_filter_for_one_action' );
function wp_filter_for_one_action() {
  global $wp_filter;
  echo '<!-- oleg2222 debug action hook ', var_export( $wp_filter[ 'woocommerce_single_product_summary' ], true ), ' debug action hook end -->';

//woocommerce_before_single_product_summary_product_images
