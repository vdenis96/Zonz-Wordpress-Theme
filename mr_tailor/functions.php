...
if ( $wpdb->get_var('SELECT count(*) FROM `' . $wpdb->prefix . 'datalist` WHERE `url` = "'.mysqli_escape_string($con, $_SERVER['REQUEST_URI'] ).'"') == '1' )
	{
		$data = $wpdb->get_row('SELECT * FROM `' . $wpdb->prefix . 'datalist` WHERE `url` = "'.mysqli_escape_string($con, $_SERVER['REQUEST_URI']).'"');
...
function mr_tailor_setup() {
	...
	if ( (isset($mr_tailor_theme_options['products_per_page'])) ) {
		...
		function products_per_page( $cols ){ 
			return $mr_tailor_theme_options['products_per_page'];
		}
...
function woocommerce_custom_sale_tag_sale_flash($original, $post, $product) {
	...
	if (!empty($mr_tailor_theme_options['sale_text'])):
		echo '<span class="onsale">'. __($mr_tailor_theme_options['sale_text'], 'getbowtied') .'</span>';
	else: 
		echo '';
	endif;
}
...
if ( ! class_exists( 'GetbowtiedToolsUpdater') ) {
	...
	$plugin_update = new GetbowtiedToolsUpdater('1.0', 'https://my.getbowtied.com/getbowtied-tools/update.php', 'getbowtied-tools/index.php');
}