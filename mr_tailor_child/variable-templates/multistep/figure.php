...
<?php 
    ...
    if (($triangle['triangle90dr'][0] == 'on') || ($triangle['triangle90dl'][0] == 'on')) { 
        ...
        get_template_part('variable-templates/multistep/figures/triangle90dl');
    }

    if ($triangle['triangle60d'][0] == 'on') { 
        set_query_var( 'gecurvd', $gecurvd );
        get_template_part('variable-templates/multistep/figures/triangle60d');
    }

    if ($triangle['triangle'][0] == 'on') { 
        get_template_part('variable-templates/multistep/figures/triangle');
    }

    if ($triangle['square'][0] == 'on') {
        set_query_var( 'triangle', $triangle ); 
        set_query_var( 'gecurvd', $gecurvd );
        get_template_part('variable-templates/multistep/figures/square');
    }
 
    if ($triangle['rectsquare'][0] == 'on') { 
        set_query_var( 'gecurvd', $gecurvd );
        get_template_part('variable-templates/multistep/figures/rectsquare');
    }
     
    if ($triangle['isquare'][0] == 'on') { 
        get_template_part('variable-templates/multistep/figures/isquare');
    }

    if ($triangle['lsquare'][0] == 'on') { 
      get_template_part('variable-templates/multistep/figures/lsquare');
    }