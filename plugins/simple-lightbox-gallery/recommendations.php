                                    ...
                                    <div id="postbox-container-2" class="postbox-container">
                                        <div id="advanced" class="meta-box-sortables">
                                            <div id="gallery_bank_get_started" class="postbox">
                                                ...
                                                <h2 class="hndle">
                                                    <span><?php esc_html_e( 'Get More Free Wordpress Plguins From Weblizar', WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></span></h3>
                                                    <div class="inside">
                                                                    ...
                                                                    <div class="wp-list-table plugin-install">
                                                                        <div id="the-list">
																			...
																			foreach ( (array) $item as $plugin ) {
																				...
																				$action_links[] = '<a href="' . esc_url( $details_link ) . '" class="thickbox" aria-label="' . esc_attr( sprintf( "More information about %s", $name ) ) . '" data-title="' . esc_attr( $name ) . '">' . esc_html__( 'More Details', WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</a>';
																				$action_links   = array();
																				if ( current_user_can( "install_plugins" ) || current_user_can( "update_plugins" ) ) {
																					...
																					switch ( $status["status"] ) {
																						case "install":
																							if ( $status["url"] ) {
																								/* translators: 1: Plugin name and version. */
																								$action_links[] = '<a class="install-now button" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( "Install %s now", $name ) ) . '">' . esc_html__( 'Install Now', WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</a>';
																							}
																							break;
																						case "update_available":
																							if ( $status["url"] ) {
																								/* translators: 1: Plugin name and version */
																								$action_links[] = '<a class="button" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( "Update %s now", $name ) ) . '">' . esc_html__( 'Update Now', WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</a>';
																							}
																							break;
																						case "latest_installed":
																						case "newer_installed":
																							$action_links[] = '<span class="button button-disabled" title="' . esc_attr__( "This plugin is already installed and is up to date", WEBLIZAR_SLGF_TEXT_DOMAIN ) . ' ">' . esc_html__(_x( 'Installed', 'plugin', WEBLIZAR_SLGF_TEXT_DOMAIN )) . '</span>';
																							break;
																					}
																				}
																				?>
																		...
																		(<?php echo number_format_i18n( $plugin["num_ratings"] ); ?>
                                                                                        ...
                                                                                        <div class="column-updated">
                                                                                            <strong><?php esc_html_e( "Last Updated:", WEBLIZAR_SLGF_TEXT_DOMAIN ); ?></strong>
																		...
																		<?php printf( "%s ago", human_time_diff( strtotime( $plugin["last_updated"] ) ) ); ?>
                                                                                        ...
                                                                                        <div class="column-compatibility">
																							...
																							if ( ! empty( $plugin["tested"] ) && version_compare( substr( $GLOBALS["wp_version"], 0, strlen( $plugin["tested"] ) ), $plugin["tested"], ">" ) ) {
																								echo '<span class="compatibility-untested">' . esc_html__( "<strong>Untested</strong> with your version of WordPress", WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</span>';
																							} elseif ( ! empty( $plugin["requires"] ) && version_compare( substr( $GLOBALS["wp_version"], 0, strlen( $plugin["requires"] ) ), $plugin["requires"], "<" ) ) {
																								echo '<span class="compatibility-incompatible">' . esc_html__( "Incompatible with your version of WordPress", WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</span>';
																							} else {
																								echo '<span class="compatibility-compatible">' . esc_html__( "Compatible with your version of WordPress", WEBLIZAR_SLGF_TEXT_DOMAIN ) . '</span>';
																							}
																							?>
                                                                                        </div>
                                                                                    </div>
