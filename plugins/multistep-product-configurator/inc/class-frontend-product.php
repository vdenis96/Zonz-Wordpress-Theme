		...
		public function add_mspc_form() {
			...
			if( mspc_enabled( $product->get_id() ) && $product->has_attributes() ) {
				...
				<div class="mspc-wrapper mspc-clearfix mspc-module-<?php echo $module; ?><?php echo $auto_next_class; echo $step_by_step_class; ?>">
					...
					<?php else: //steps, tabs, vertical steps ?>
						...
						<div class="mspc-content ui <?php echo $this->get_content_class($module); ?>">
							...
							while($attribute_count < $loop_length):
								...
								?>
								<?php
								/* CUSTOM get taxonomy description */
								
								$taxonomy = str_replace('pa_', '', $attr_key);
								global $wpdb;
								$attribute_id = $wpdb->get_var("select attribute_id from {$wpdb->prefix}woocommerce_attribute_taxonomies where attribute_name='".$taxonomy."'");
								
								$taxonomy_description = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM " . $wpdb->prefix . "yith_wccl_meta WHERE wc_attribute_tax_id = %d", $attribute_id ) );
								?>
								
								<div class="mspc-tab-content mspc-variations mspc-clearfix ui column grid doubling mspc-<?php echo $attr_key. ' '.$this->get_column_class($columns); ?>">
									<div class="custom_text custom_text_top"><?php echo $taxonomy_description; ?></div>
									<div class="flex-row">
										
										<div class="large-12">
											<div class="steps_custom_container">
												<?php
												echo $this->get_variation_items(
													$attr_key,
													$options,
					...
					<?php endif; ?>
					<a href="#" class="mspc-clear-selection"><?php _e( 'Wis mijn keuzes en begin opnieuw.', 'radykal' ); ?></a>
