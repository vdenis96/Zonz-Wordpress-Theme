...
class Permalink_Manager_Pro_Functions extends Permalink_Manager_Class {
	...
	public static function get_expiration_date($basic_check = false, $empty_if_valid = false) {
		...
		$exp_date = (!empty($permalink_manager_options['licence']['expiration_date'])) ? (int) $permalink_manager_options['licence']['expiration_date'] : false;
		...
		// Lifetime license key
			...
			$expiration_info = sprintf(__('Your licence key is valid until %s.<br />To prolong it please go to <a href="%s" target="_blank">this page</a> for more information.', 'permalink-manager'), date(get_option('date_format'), $exp_date), 'https://permalinkmanager.pro/docs/billing/how-to-renew-the-license-key/');
			$expired = 0;
		} else {
			$expiration_info = sprintf(__('Please paste the licence key to access all Permalink Manager Pro updates & features <a href="%s" target="_blank">on this page</a>.', 'permalink-manager'), $settings_page_url);
			$expired = 2;
