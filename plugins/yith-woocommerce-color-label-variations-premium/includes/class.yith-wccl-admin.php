	...
	class YITH_WCCL_Admin {
		...
		public function __construct() {
			...
			$this->_custom_tax = array(
				'colorpicker' => __( 'Colorpicker', 'ywcl' ),
				'image'       => __( 'Image', 'ywcl' ),
				'label'       => __( 'Label', 'ywcl' )
			);
		...
		public function action_links( $links ) {
			$links[] = '<a href="' . admin_url( "admin.php?page={$this->_panel_page}" ) . '">' . __( 'Settings', 'ywcl' ) . '</a>';
		...
		public function register_panel() {
			...
			$admin_tabs = array(
				'general' => __( 'Settings', 'ywcl' )
			);

			$args = array(
				'create_menu_page' => true,
				'parent_slug'      => '',
				'page_title'       => __( 'Color and Label Variations', 'ywcl' ),
				'menu_title'       => __( 'Color and Label Variations', 'ywcl' ),
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( defined( 'YITH_WCCL_INIT') && YITH_WCCL_INIT == $plugin_file ) {
				$plugin_meta[] = '<a href="' . $this->doc_url . '" target="_blank">' . __( 'Plugin Documentation', 'ywcl' ) . '</a>';
			}
		...
		public function attribute_add_description_field( $id, $attribute ) {
			...
			//$descr = isset( $_POST['attribute_description'] ) ? wc_clean( $_POST['attribute_description'] ) : '';			
                        // Allow HTML in description by remove wc_clean function
			$descr = isset( $_POST['attribute_description'] ) ? ( $_POST['attribute_description'] ) : '';
                        // End Allow HTML
		...
		public function product_option_terms( $taxonomy, $i ) {
			...
			<button class="button fr plus yith_wccl_add_new_attribute" data-type_input="<?php echo $taxonomy->attribute_type ?>"><?php _e( 'Add new', 'ywcl' ); ?></button>
		...
		public function product_option_add_terms_form() {
			...
			<div id="yith_wccl_dialog_form" title="<?php _e('Create new attribute term','ywcl')?>" style="display:none;">
				<span class="dialog_error"></span>
				<form>
					<fieldset>
						<label for="term_name"><?php _e( 'Name', 'ywcl' ) ?>:
							<input type="text" name="term_name" id="term_name" value="" >
						</label>
						<label for="term_slug"><?php _e( 'Slug', 'ywcl' ) ?>:
							<input type="text" name="term_slug" id="term_slug" value="">
						</label>
						<label for="term_value"><?php _e( 'Value', 'ywcl' ); ?>:
							<input type="text" name="term_value" id="term_value" value="" data-type="label">
						</label>
						<label for="term_tooltip"><?php _e( 'Tooltip', 'ywcl' ); ?>:
		...
		public function yith_wccl_add_new_attribute_ajax() {
			...
			if( $value == '' ) {
				wp_send_json( array(
					'error' => __( 'A value is required for this term', 'ywcl' )
				) );
			}
