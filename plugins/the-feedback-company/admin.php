...
class feedbackcompany_admin_settings
{
	...
	public function __construct()
	{
		...
		// special action for handling TXT file download and deletion of the error log
	...
	function init_assets($hook)
	{
		...
		wp_enqueue_script('feedbackcompany_admin_javascript', plugins_url('js/admin.js', __FILE__));
		...
		wp_enqueue_style('feedbackcompany_admin_stylesheet', plugins_url('css/admin.css', __FILE__));
	}
	...
	public function init_settings()
	{
		...
		$this->option_set_default('feedbackcompany_wordpressmultilanguage', 'all');
		$this->option_set_default('feedbackcompany_oauth_client_id', '');
		$this->option_set_default('feedbackcompany_oauth_client_secret', '');
		$this->option_set_default('feedbackcompany_invitation_enabled', 1);
		$this->option_set_default('feedbackcompany_invitation_orderstatus', 'wc-completed');
		$this->option_set_default('feedbackcompany_invitation_delay', '7');
		$this->option_set_default('feedbackcompany_invitation_delay_unit', 'days');
		$this->option_set_default('feedbackcompany_invitation_reminder_enabled', 1);
		$this->option_set_default('feedbackcompany_invitation_reminder', '14');
		$this->option_set_default('feedbackcompany_invitation_reminder_unit', 'days');
		$this->option_set_default('feedbackcompany_mainwidget_size', 'small');
		$this->option_set_default('feedbackcompany_mainwidget_amount', 0);
		$this->option_set_default('feedbackcompany_stickywidget_enabled', 0);
		$this->option_set_default('feedbackcompany_productreviewsextendedwidget_displaytype', 'sidebar');
		$this->option_set_default('feedbackcompany_productreviews_enabled', false);

		// name of our page
		$page = 'feedbackcompany_settings';

		// define settings fields
		$fields = array();
		$fields['section_oauth'] = array('section', 'Authentication Settings');
		$fields['feedbackcompany_oauth_client_id'] = array('text', 'Client ID');
		$fields['feedbackcompany_oauth_client_secret'] = array('text', 'Client secret');

		// WPML settings only if WPML is enabled
		if (feedbackcompany_wordpressmultilanguage_enabled())
		{
			$fields['section_wpml'] = array('section', 'Choose your active language');
			$fields['feedbackcompany_wordpressmultilanguage'] = array('wpml', 'Language');
		}

		// woocommerce settings only if woocommerce is enabled
		if (feedbackcompany_woocommerce_enabled())
		{
			$fields['section_woocommerce'] = array('section', 'WooCommerce Settings');
			$fields['feedbackcompany_invitation_enabled'] = array('boolean', 'Send review invitation');
			$fields['feedbackcompany_invitation_orderstatus'] = array('orderstatus', 'Order status');
			$fields['feedbackcompany_invitation_delay'] = array('timeframe', 'Invitation delay');
			$fields['feedbackcompany_invitation_reminder_enabled'] = array('boolean', 'Send review reminder');
			$fields['feedbackcompany_invitation_reminder'] = array('timeframe', 'Reminder delay');
			if (get_option('feedbackcompany_productreviews_enabled'))
			{
				$fields['section_productreviewswidget'] = array('section', 'Product reviews widget');
				$fields['feedbackcompany_productreviewsextendedwidget_displaytype'] = array('displaytype', 'Product page widget');
				$fields['feedbackcompany_productreviewsextendedwidget_toggle_element'] = array('text', 'Widget toggle element');
			}
			else
			{
				$fields['section_productreview'] = array('section', 'Product reviews disabled');
			}
		}
		$fields['section_mainwidget'] = array('section', 'Badge widget');
		$fields['feedbackcompany_mainwidget_size'] = array('size', 'Size');
		$fields['feedbackcompany_mainwidget_amount'] = array('amount', 'Amount of reviews');
		$fields['feedbackcompany_mainwidget_shortcode'] = array('shortcode', 'Shortcode', 'feedbackcompany_badge');
		$fields['feedbackcompany_mainwidget_preview'] = array('widgetpreview', 'Preview');

		$fields['section_barwidget'] = array('section', 'Bar widget');
		$fields['feedbackcompany_barwidget_shortcode'] = array('shortcode', 'Shortcode', 'feedbackcompany_bar');
		$fields['feedbackcompany_barwidget_preview'] = array('widgetpreview', 'Preview');

		$fields['section_stickywidget'] = array('section', 'Floating widget');
		$fields['feedbackcompany_stickywidget_enabled'] = array('boolean', 'Enabled');
		$fields['feedbackcompany_stickywidget_preview'] = array('widgetpreview', 'Preview');
	...
	public function downloaderrorlog()
	{
		...
		header("Content-Disposition: attachment; filename=feedbackcompany-errorlog.txt");
	...
	/**
	 * This function outputs the requested admin page, current or legacy
	 *
	 * This code can be dropped once legacy support is
	 */
	public function output_adminpage()
	{
		if (isset($_GET['legacywoocommerce']))
			return $this->output_adminpage_legacywoocommerce();
		if (isset($_GET['legacywidgets']))
			return $this->output_adminpage_legacywidgets();

		return $this->output_adminpage_current();
	}

	/**
	 * Output function for the settings page of the current version of this plugin
	 */
	public function output_adminpage_current()
	{
		echo '<div class="wrap">';
		...
		if (!get_option('feedbackcompany_oauth_client_id') || !get_option('feedbackcompany_oauth_client_secret'))
		{
		...
		elseif (!feedbackcompany_api_wp()->ext->get_option('feedbackcompany_access_token'))
		{
		...
		do_settings_sections('feedbackcompany_settings');
		...
		{
			echo '<a class="button-secondary" href="?page=feedbackcompany&downloaderrorlog">Download log file</a>';
			echo '<a class="button-secondary" href="?page=feedbackcompany&deleteerrorlog">Delete log file</a>';
		}
		echo '</p>';

		echo '</form>';

		// buttons for legacy config, if legacy widgets or woocommerce is enabled

		if (feedbackcompany_legacywidgets_enabled() || feedbackcompany_legacywoocommerce_enabled())
		{
			echo '<h1>Legacy configuration for 1.x versions of this plugin</h1>';
			echo '<p class="max600px">';
			echo 'Your Wordpress installation contains configuration for an older version of this plugin.  The 1.x versions are no longer developed, '
				. 'but functionality is integrated into the current version of this plugin for backwards compatibility. '
				. '<br><br>Everything still works like it did (for now) and configuration is available '
				. 'via the links below. Support for these versions will be dropped in the future however, '
				. 'so please migrate to the new version using the configuration screen above when you can.';
			echo '</p>';
			if (feedbackcompany_legacywidgets_enabled())
				echo '<a class="button-secondary" href="?page=feedbackcompany&legacywidgets">Legacy Review Widgets configuration</a> &nbsp; ';
			if (feedbackcompany_legacywoocommerce_enabled())
				echo '<a class="button-secondary" href="?page=feedbackcompany&legacywoocommerce">Legacy WooCommerce configuration</a> &nbsp; ';
		}
	...
	{
		$tooltips = array(
			'section_wpml' => 'Selecting a specific language will cause the Feedback Company widgets and order registration to only be active for that language.',
			...
			'section_productreviewswidget' => 'Configure how to display your product reviews widget.Visit https://feedbackcompany.wixanswers.com for more information.',
			...
			'section_productreview' => 'Here you can find the state and information of product review widgets',
		);

		echo '<span class="dashicons dashicons-editor-help" title="'.$tooltips[$section['id']].'"></span>';
		echo '<br style="clear: both;">';

		// oauth has notice added if succesfully connected
		if ($section['id'] == 'section_oauth')
		{
			if (feedbackcompany_api_wp()->ext->get_option('feedbackcompany_access_token'))
				echo '<span class="feedbackcompany-notice-ok">&#10004; Your Wordpress is connected to Feedback Company.</span>';
		}
		if ($section['id'] == 'section_productreview')
		{
	...
	public function field_output_wpml($args)
	{
		$options = array('all' => 'All languages');
		$active_languages = apply_filters('wpml_active_languages', NULL);
		foreach ($active_languages as $language_key => $language_value)
		{
			$options[$language_key] = $language_value['translated_name'];
		}
		$this->field_output_selectbox($args, $options);
	}
	...
	/**
	 * Output function for legacy WooCommerce configuration screen
	 */
	public function output_adminpage_legacywoocommerce()
	{
		echo '<div class="wrap">';
		echo '<h1>Feedback Company Legacy Configuration - WooCommerce</h1>';
		echo '<div class="notice notice-warning"><p>'
			. '<strong>Warning!</strong>  The legacy WooCommerce connector is no longer developed and support will be dropped in a future version of this plugin. '
			. 'Please use the new version 2.3.1 order registration and product reviews whenever you can.'
			. '</p></div>';

		global $feedbackcompany_woocommerce;
		if ($feedbackcompany_woocommerce)
			$feedbackcompany_woocommerce->fc_settings_display();
		else
			echo 'Congratulations! The older, no longer supported WooCommerce connector is disabled on this Wordpress installation '
				. 'which means you are ready to use the shiny new order registration and product reviews widgets.'
				. '<br><br>'
				. '<a class="button-primary" href="?page=feedbackcompany">Click here to go the configuration of the current WooCommerce extension</a>';
		echo '</div>';
	}

	/**
	 * Output function for legacy widgets configuration screen
	 */
	public function output_adminpage_legacywidgets()
	{
		echo '<div class="wrap">';
		echo '<h1>Feedback Company Legacy Configuration - Review Widgets</h1>';
		echo '<div class="notice notice-warning"><p>'
			. '<strong>Warning!</strong>  Legacy widgets are no longer developed and support will be dropped in a future version of this plugin. '
			. 'Please use the new version 2.3.1 widgets whenever you can.'
			. '</p></div>';

		global $feedbackcompany_legacywidget_adminsettings;
		if ($feedbackcompany_legacywidget_adminsettings)
			$feedbackcompany_legacywidget_adminsettings->output_adminpage();
		else
			echo 'Congratulations! The older, no longer supported legacy widgets are disabled on this Wordpress installation '
				. 'which means you are ready to use the shiny new widgets.'
				. '<br><br>'
				. '<a class="button-primary" href="?page=feedbackcompany">Click here to go the configuration of the current widgets</a>';
		echo '</div>';
	}
	...
	public function updatedsettings()
	{
		...
		if ($this->updated_widgetmain)
		{
			feedbackcompany_api_wp()->register_widget_main(
				get_option('feedbackcompany_mainwidget_size'),
				get_option('feedbackcompany_mainwidget_amount'),
				$force_refresh
			);
		}
		...
		if ($this->updated_widgetproductsummary && get_option('feedbackcompany_productreviews_enabled'))
		{
			feedbackcompany_api_wp()->register_widget_productsummary($force_refresh);
		}

		if ($this->updated_widgetproductextended && get_option('feedbackcompany_productreviews_enabled'))
		{
			feedbackcompany_api_wp()->register_widget_productextended(
				get_option('feedbackcompany_productreviewsextendedwidget_displaytype'),
				$force_refresh
			);
		}
...
add_action('update_option_feedbackcompany_oauth_client_id', array($feedbackcompany_admin_settings_page, 'updated_oauth'), 10, 2);
add_action('update_option_feedbackcompany_oauth_client_secret', array($feedbackcompany_admin_settings_page, 'updated_oauth'), 10, 2);
add_action('update_option_feedbackcompany_mainwidget_size', array($feedbackcompany_admin_settings_page, 'updated_widgetmain'), 10, 2);
add_action('update_option_feedbackcompany_mainwidget_amount', array($feedbackcompany_admin_settings_page, 'updated_widgetmain'), 10, 2);
add_action('update_option_feedbackcompany_productreviewsextendedwidget_displaytype', array($feedbackcompany_admin_settings_page, 'updated_widgetproductextended'), 10, 2);
// register action to update widget settings on shutdown
add_action('shutdown', array($feedbackcompany_admin_settings_page, 'updatedsettings'), 10, 2);
