...
class WC_Custom_Payment_Gateway_1 extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title   = __( 'ZONZ CONTROLEERT ALLEREERST JOUW BESTELLING', 'wcwcCpg1' );
	...
	function process_payment ($order_id, $z=0) {
		global $woocommerce;
		...
		// Mark as on-hold
        if ($z != 0) {
            $order->update_status('beoordeelbestelli', __( 'Your order wont be shipped until the funds have cleared in our account.', 'woocommerce' ) );
        } else {
