    ...
    public function CreateQuote()
    {
        ...
        if (sizeof($this->orderData->get_items()) > 0) {
            foreach ($this->orderData->get_items() as $item) {
                ...
                $row = array(
                    'name' => $item['name'],
                    'custom_figure'     => $item['custom_figure'],
                ...
                foreach ( WC()->cart->get_cart() as $cart_item_key => $item ) {
                   ...
                   $attr =  $_SESSION['productAttr'][$item_key];
               }

                // Check if we need to display product ID/SKU
                $item_area = $item['area']['area'];
                $item_name = $row['name'];
                $item_data = $item['data'];
                //echo $item_data;
                //$item_area = $cart_item['area']['area'];
...
//oleg  - to fix  custom_figure ???                 
					//if ($row['custom_figure']) {
                    if (true) {
                        ...
                        $item_name = str_replace("*", "", $item_name);
                        $r_count = substr_count($item_name, "\n");
                        $rtemp=12;

//oleg - white padding below all items under Product, above meta
                        //$r= $r_count * $rtemp;
                        $r= $r_count * $rtemp + 20; //oleg +20
                   }

                    // Name
                    //$this->MultiCell(280, $r,$this->decode($item_name),$this->showBorders,'L', 0, 1, 40, $y, true, 0, false, false, $r, 'C', false);
                    //$this->MultiCell(280, $r, $this->decode($item_name), $this->showBorders, 'L', 0, 1, 40, $y, true, 0, false, false, $r, 'T', false);
////////			  $this->MultiCell(280, $r, $this->decode($item_name),"\n",$this->decode($item_area), $this->showBorders, 'L', 2 , 1, 50, $y, true, 0, false, false, $r, 'T', false);


//oleg	y=w r=h		//$this->MultiCell(280, $r, $item_name, $this->showBorders, 'L', 0, 1, 40, $y, true, 0, false, false, $r, 'T', false);
                      $this->MultiCell(280, $r, $this->decode($item_name), $this->showBorders, 'L', 0 , 1, 40, $y, true, 0, false, false, $r, 'T', false);
//     MultiCell(w,   h,  txt,  border = 0,             align = 'J', fill = 0, ln = 1, x = '', y = '', reseth = true, stretch = 0, ishtml = false, autopadding = true, maxh = 0) 
                    ...
                    if (!empty($row['meta'])) {
                        $y += $r + 2;
                        $this->style('gray');
//oleg comment - meta is ok - gray text  like Hdpe 230gr (Anet 940 hdpe 230 gr), Kleur: Anet 940-06
                        $this->MultiCell(515, 0, $this->decode($row['meta']), $this->showBorders, 'L', 0, 1, 45, $y, true, 0, false, false, 0, 'T', false);
                        $y += $current_meta_height;
                        $this->style();
                    }
    ...
    public function render_totals($y, $r, $s)
    {
        ...
        // Draw top border
//oleg - $this->Line(40, ($y-2), 560, ($y-2), array('color' => array(192, 192, 192)));
        $this->Line(40, ($y-2), 560, ($y-2), array('color' => array(1, 1, 1)));
        $y += 10;
...
//oleg comment - shows taxes title and taxes amount
...
//oleg comment and test - totals boxes border color 
            $borders_name = ($key != 'total') ? $this->showBorders : array('LTB' => array('width' => 1, 'color' => array(0, 0, 0))); //test green array(0, 192, 0)
            $borders_value = ($key != 'total') ? $this->showBorders : array('TRB' => array('width' => 1, 'color' => array(0, 0, 0))); //test blue array(0, 0, 192)
...
//oleg test - ? this gives black background for totals?            
			//$this->MultiCell(140, $r, $this->decode($name), $borders_name, 'L', 1, 1, 240, $y, true, 0, false, $auto_padding, $r, 'T', false);
